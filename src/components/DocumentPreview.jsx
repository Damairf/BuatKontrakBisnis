  ...
  return (
    <div className="pl-6 sm:pl-12 mb-3">
      ...
      <p className="text-[12pt] text-justify mt-2 leading-relaxed">
        ...
        (selanjutnya disebut sebagai <strong>“{label}”</strong>){isLast ? ";" : ";"}
      </p>
    </div>
  );
}
...
function Block({ block }) {
  switch (block.type) {
    case "title":
      return <h1 className="text-center font-bold text-[14pt] my-6 leading-normal uppercase">{block.text}</h1>;

    case "para":
      return <p className="text-[12pt] text-justify my-4 leading-relaxed">{block.text}</p>;

    case "partyblock":
      return <PartyBlock label={block.label} pihak={block.pihak} isLast={block.isLast} />;
    ...
    case "pasal":
      return (
        <div className="text-center mt-8 mb-4">
          <p className="font-bold text-[12pt] leading-normal">Pasal {block.nomor}</p>
          ...
          {block.groups.map((g, i) => (
            <li key={i} className="leading-relaxed pl-1">
              ...
              <ol className="list-[lower-alpha] list-outside pl-5 mt-1.5 space-y-1.5">
                {g.items.map((it, j) => (
                  <li key={j} className="leading-relaxed pl-1">{it}</li>
                ))}
              </ol>
            </li>
          ))}
    ...
    case "signature":
      return (
        <div className="grid grid-cols-2 gap-4 mt-12 text-center text-[12pt]">
          <div>
            <p className="mb-20">{block.kiri.label}</p>
            <p className="font-semibold underline underline-offset-4">{block.kiri.nama}</p>
          </div>
          <div>
            <p className="mb-20">{block.kanan.label}</p>
            <p className="font-semibold underline underline-offset-4">{block.kanan.nama}</p>
          </div>
        </div>
      );
